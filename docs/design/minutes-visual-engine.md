...
- LLM / 启发式 **只填 IR**，React **只渲染 IR**  
- 与现有 `MinutesDoc` **并存、可降级**
...
| 5 | **可降级** | 无 LLM / JSON 坏 / 空 board → 静默不展示或启发式 board，不挡纪要 |
| 6 | **视觉本仓化** | tone → DESIGN token；禁止模型指定 hex/字体 |
| 7 | **显式优先（首版）** | 图解可与 Auto Minutes 解耦：默认「纪要就绪后生成/刷新图解」，避免拖垮主路径 |
...
│  B. Heuristic：从 MinutesDoc 字段确定性投影（mock/无 key）      │
└───────────────────────────┬─────────────────────────────────┘
                            ▼
┌─────────────────────────────────────────────────────────────┐
...
| 触发 | 首版建议 |
|---|---|
| Auto Minutes 成功后 | **不默认**连环生成图解（省延迟/失败面） |
| 纪要 Tab「生成图解 / 刷新图解」 | **P0 路径** |
| 用户保存结构化纪要后 | 可选「图解过期」角标，不自动重跑 |
| mock / 无 API key | Heuristic filler，保证 UI 可演示 |
