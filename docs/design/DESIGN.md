# DESIGN.md — meeting-record UI

> **本仓视觉真源。** 实现 UI / 写前端时以本文件为准。  
> **来源：** 自 `docs/design/reference/repos/awesome-design-md/design-md/claude/DESIGN.md` 采纳（Claude 暖色编辑气质）。  
> **决策：** [ADR 0002](../adr/0002-ui-design-claude.md) · 说明见 [reference/ui.md](./reference/ui.md)。  
> **禁止：** 再混用 Linear / Notion 等第二品牌 token；不要在实现里直接依赖 `repos/awesome-design-md`。

## 本仓适配（读 token 前先读）

| 面 | 怎么用 Claude 气质 |
|---|---|
| 应用壳 | `canvas` 奶油底 + `ink` 正文；导航克制，少大色块 |
| 会议列表 | `surface-card` / `surface-soft` 卡片；状态用 muted + 少量 primary |
| 转写全文 | **长阅读优先**：body 行高 ≥1.55；时间戳/说话人用 `muted` / `caption` |
| 纪要 / 待办 | 结构化块用卡片层级；CTA（生成纪要、导出）用 `primary` 珊瑚 |
| 任务进行中 | 转写排队/进度：可用 `accent-teal` / `accent-amber` 作状态，勿满屏 primary |
| 代码/原始 JSON | `code` 字体；必要时局部 `surface-dark` 面板（转写调试） |
| 营销落地页 | 可用 serif display；**应用内主界面以 sans title/body 为主**，避免每行都是 display 衬线 |

字体回退（无商业字时）：

- Display serif → `Georgia, "Noto Serif SC", serif`（中文场景优先系统衬线/思源）
- UI sans → `Inter, "PingFang SC", "Microsoft YaHei", sans-serif`
- Mono → `JetBrains Mono, ui-monospace, monospace`

以下为上游 Claude 分析正文（tokens / 规则），实现时与上表一并遵守。
